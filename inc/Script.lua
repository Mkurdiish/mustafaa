...
bd = '🙋🏻‍♂╿العضو ['..USERNAME..'](tg://user?id='..msg.sender_user_id_..') \n📌╽لأستخدام البوت عليك الاشتراك ب قناة السورس https://t.me/tweakjailbreak'
...
return "📛*│* هذا امر يخص {المدير,المنشئ,المطور} فقط  \n🚶"
end
...
function download_to_fileh(url, file_path) 
    ...
    redis:set(max.."dl_yt_dl"..msg.chat_id_,"close") 
    ...
    redis:set(max.."dl_yt_dl"..msg.chat_id_,"open") 
    return '\n💢┇تم تفعيل اليوتيوب في المجموعه ...'
    end
    
    if MsgText[1] and MsgText[2] == "!!" and redis:get(max.."dl_yt_dl"..msg.chat_id_) == "open" then
    ...
    if k == 1 then
    local GetStart = io.popen('downloadsh '..vv.id):read('*all')
    if GetStart and GetStart:match('(.*)oksend(.*)') then
    print('download Mp3 done ...\nName : '..vv.Title..'\nIdLink : '..vv.id)
    sendAudio(msg.chat_id_, msg.id_,'./'..vv.id..'.mp3',vv.Title,vv.Title)
    ...
    if GetStart and GetStart:match('(.*)oksend(.*)') then
    print('download Mp3 done ...\nName : '..vv.Title..'\nIdLink : '..vv.id)
    sendVoice(msg.chat_id_,msg.id_,'./'..vv.id..'.mp3',vv.Title)
    os.execute('rm -rf ./'..vv.id..'.mp3') 
    end
    end
    end
...
if not msg.Creator then return "♦️*│*هذا الامر يخص {المطور,المنشئ} فقط  \n💥" end
redis:setex(max..'about:witting'..msg.chat_id_..msg.sender_user_id_user_id_,300,true) 
...
return [[
┄─┅══┅─┄     
👋│اهلا بك في سورس مصطفى❤️

🧔🏻|[مطور الـسـورس](https://t.me/alone_kurdish)
📨│ [قناه السورس](https://t.me/tweakjailbreak) 
📨| [قروب السورس](https://t.me/iOSSSJailbreak)
